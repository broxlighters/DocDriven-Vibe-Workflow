...
**只保留「在谈的」需求，定稿即归档**：一个需求确认并建成 RQ 后，把它从「讨论稿」移除（**项目共识区不动**）。已定稿需求不在 discovery.md 留索引——真相在 Requirements 表的 RQ，增量模式要知道「已定稿了哪些模块」直接查该表。这样 discovery.md 的体量只随**未决需求数**变化，不随项目历史无限膨胀；项目背景始终在「项目共识」区手边。
...
> JSON 参数（`--json` 写入、`--filter-json` 查询）都不要内联——PowerShell 会剥引号/拆参致写入失败或查询先失败再回退。一律写到项目目录临时文件用 `@文件` 传参：**写入用 `@./.lark_tmp.json`，查询用 `@./.lark_filter.json`**，用完 `rm -f`。详见 `docs/lark-base.md`「JSON 参数」。

```bash
cat > ./.lark_filter.json <<'EOF'
{"logic":"and","conditions":[["Requirement","==","RQ-XXX"]]}
EOF
...
{"TaskID":"TASK-001","Title":"实现登录接口","Status":"todo","Requirement":[{"id":"$REQ_REC"}],"Dependencies":"","Priority":"HIGH","FailCount":0,"Goal":"实现JWT登录","AcceptanceCriteria":"POST /login\n返回JWT\n返回RefreshToken","Modules":"AuthController\nAuthService"}
EOF
...
```bash
cat > ./.lark_tmp.json <<'EOF'
{"Status":"coding","FailCount":1,"ReviewResult":"FAIL","ReviewRound":1,"ReviewProblems":"RefreshToken未设置过期时间\n缺少单元测试","ReviewSuggestions":"补充Token配置\n增加测试"}
EOF
lark-cli base +record-upsert --as bot --profile $LARK_PROFILE --base-token $LARK_APP_TOKEN --table-id $TASKS_TABLE_ID \
