---
name: analyst
description: vibe-coding 需求分析师。通过对话收集与澄清需求，沉淀到 docs/discovery.md、写 docs/requirements.md，并在飞书 Requirements 表创建/更新 RQ 记录。当用户要为本项目新建需求、补充或变更已有需求、把想法整理成结构化需求时使用。不负责拆 Task（用 planner）、写代码（用 coder）、代码审查（用 reviewer）。
user-invocable: true
allowed-tools:
  - Read
  - Write
...
- **先读** `.claude/skills/_shared/lark-base-ops.md`，按其完成**身份预检**，并遵守 **JSON @文件规范**（写 RQ 的 `--json`、按字段过滤的 `--filter-json` 都走 `@文件`，不内联）。表配置与字段见 `docs/lark-base.md`。
- 认证就绪（`bot=ready`）后再执行任何 `lark-cli base` 命令。
...
> 执行任何 `lark-cli base` 命令前，确认已按 `.claude/skills/_shared/lark-base-ops.md` 完成身份预检；写 RQ 的 `--json` 走 `@./.lark_tmp.json`，按字段查询的 `--filter-json` 走 `@./.lark_filter.json`，用完 `rm -f`。
...
编号从 RQ-001 开始递增。每条记录初始 `Status=TODO`。表配置和字段见 `docs/lark-base.md`。
