...
- **先读** `.agents/skills/_shared/lark-base-ops.md`，按其完成**身份预检**（先于一切 `lark-cli base` 命令），并遵守 **JSON @文件规范**（拆 Task / 建 RQ / 改状态的 `--json` 和按字段过滤的 `--filter-json` 都走 `@文件`，不内联；写入用 `./.lark_tmp.json`，查询用 `./.lark_filter.json`，删除类加 `--yes`）。
