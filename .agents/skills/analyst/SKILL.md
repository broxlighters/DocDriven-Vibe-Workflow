...
- **先读** `.agents/skills/_shared/lark-base-ops.md`，按其完成**身份预检**，并遵守 **JSON @文件规范**（写 RQ 的 `--json`、按字段过滤的 `--filter-json` 都走 `@文件`，不内联）。表配置与字段见 `docs/lark-base.md`。
...
> 执行任何 `lark-cli base` 命令前，确认已按 `.agents/skills/_shared/lark-base-ops.md` 完成身份预检；写 RQ 的 `--json` 走 `@./.lark_tmp.json`，按字段查询的 `--filter-json` 走 `@./.lark_filter.json`，用完 `rm -f`。
